...
pwd = os.getcwd()
all_chapters_path = glob(pwd + "/source/ch0*")
...
def generate_mapping():
    ...
    for key in mapping.keys():
        chapter_file = os.path.join(pwd, "source", "chapters", key + ".rst")
...
def get_toc_info():
    ...
    for dir_path in all_chapters_path:
        ...
        for file in files:
            ...
            section = int(re.findall(r"ch\d{2}_(\d{2}).md", file_name)[0])
            md_path = "https://effectivepython.readthedocs.io/zh_CN/latest/" + dir_name + "/" + file_name.replace("md", "html")
